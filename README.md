# 个人修改版
（注意:我只对微信经营商户进行了优化）我在使用原版安卓监控的时候发现了一个问题，针对于商户收款和安卓15收款会出现不回调或者将顾客来访次数当做收款金额（比如老顾客来访时100次会将金额默认成100.00）,我针对这种进行了优化,在编译完之后，觉得做开源项目上传贡献也不错，所以就上传了这个，另外没有改任何东西（所以界面右上角为原作者信息），后面会考虑进行其他优化，莫名其妙把日志砍掉了不会修（零基础代码大部分靠ai写的😭😭）
